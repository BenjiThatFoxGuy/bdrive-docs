...
3. **Direct-parts creation**: a file can also be created by supplying a `parts` array with **no** `uploadId`, bypassing the upload-and-hash flow above. In that case the server best-effort computes a hash at creation time by reading the file's just-uploaded parts back from Telegram. This is a real, API-exposed creation path (not just a test fixture), so files created this way are hashed too — but because it requires an extra round-trip read of the content, a failure here is logged and swallowed rather than failing the file creation. Clients should send `uploadId` to stay on path 1 and avoid the read-back entirely.

   Uploading with `hashing=false` deliberately leaves no block hashes on record, and is honoured rather than worked around: such a file is created without a hash and left for path 4, instead of being read back from Telegram.
...
### Chunk size must be a multiple of 16MB

The tree hash is built from 16MB block hashes, and on paths 1 and 3 the server hashes each uploaded part independently before concatenating the results. That reproduces the hash of the whole file — the value a client computes locally, and the value deduplication matches on — **only when every part is a whole number of 16MB blocks**.

A client whose chunk size splits a block will hash identical content to a different value than a client whose chunk size doesn't, and the two uploads will never be recognised as duplicates of each other. Files small enough to fit in a single part are unaffected, since there is then only one part to hash. Both the web UI and the rclone/bclone backend round their configured chunk size to the nearest 16MB multiple for this reason; third-party clients must do the same.

## Behavior Notes and Limitations

- **Copy-on-write**: if a file's content is later modified while it has a `referencedFileId` set, the reference is cleared (set back to `NULL`) and the file becomes its own canonical copy. If the new content happens to match a *different* existing file's hash, it's re-linked to that file instead.
- **Canonicals are preferred when linking**: when several files share a hash, a new duplicate is linked to one that owns its own Telegram parts rather than to another reference, so `referencedFileId` chains don't form. Only rows of type `file` are considered.
